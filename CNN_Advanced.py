...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 27 13:06:45 2018
...
def variable_with_weight_loss(shape, stddev, w1):
    ...
    if w1 is not None:
        weight_loss = tf.multinomial(tf.nn.l2_loss(var), w1, name='weight_loss')
...
images_test, labels_test = cifar10_input.imputs(eval_data=True,
                                                data_dir=data_dir,
...
# defintion dense layer
reshape = tf.reshape(pool2, [batch_size, -1])
dim = reshape.get_shape()[1].value
weight3 = variable_with_weight_loss(shape=[dim,384], stddev=0.04, w1=0.004)
bias3 = tf.Variable(tf.constant(0.1, shape=[384]))
loacl3 = tf.nn.relu(tf.matmul(reshape, weight3) + bias3)

# defintion dense later 2
weight4 = variable_with_weight_loss(shape=[384, 192], stddev=0.04, w1=0.004)
bias4 = tf.Variable(tf.constant(0.1, shape=[192]))
local4 = tf.nn.relu(tf.matmul(loacl3, weight4) + bias4)
...
for step in range(max_steps):
    ...
    if  step % 10 == 0:
        ...
        format_str = ('step %d, loss=%.2f (%.1f exxamples/sec; %.3f sec/batch)')
